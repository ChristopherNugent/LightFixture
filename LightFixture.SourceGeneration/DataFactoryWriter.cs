...
internal sealed class DataFactoryWriter
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (factoryDefinition.ObsoleteHandling is ObsoleteHandlingBehavior.SuppressWarnings
            or ObsoleteHandlingBehavior.SuppressErrors)
        {
            code.AppendLine(DisableObsoleteWarningsPragma);
        }
...
#pragma warning restore CS0612 // Type or member is obsolete
            ...
            if (factoryDefinition.ObsoleteHandling is ObsoleteHandlingBehavior.SuppressErrors)
            {
                code.AppendLine(ObsoleteAttributeSource);
            }
        ...
        ITypeSymbol type)
    {
        ...
        while (currentType is not null)
        {
            ...
            foreach (var member in members)
            {
                ...
                if (definition.ObsoleteHandling is ObsoleteHandlingBehavior.IgnoreObsolete
                    && GetObsoleteType(property) is not ObsoleteType.None)
                {
                    continue;
                }
    ...
    private static ObsoleteType GetObsoleteType(IPropertySymbol propertySymbol)
    {
        foreach (var attribute in propertySymbol.GetAttributes())
        {
            if (attribute.AttributeClass?.ToDisplayString() is not WellKnownTypes.ObsoleteAttribute)
            {
                continue;
            }

            var isError = attribute.ConstructorArguments.Length is 2
                          && attribute.ConstructorArguments[1].Value is true;

            return isError ? ObsoleteType.Error : ObsoleteType.Warning;
        }

        return ObsoleteType.None;
    }
